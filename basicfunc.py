...
def sum_of_gaussians(x, *params):
    ...
    for i in range(n):
        ...
        mu = params[3*i + 1]
        sigma = params[3*i + 2]
        result += gaussian(x, amp, mu, sigma)
    return result

# GAUSSIAN MULTIPLE PEAKS FIT
def fit_spectrum_with_multiple_gaussians(data=None, bin_centers=None, counts=None,
                                         bins=None, prominence=None, plot=True):
    if data is not None:
        if bins is None:
            bins = calculate_bins(data)
        counts, bin_edges = np.histogram(data, bins=bins)
        bin_centers = (bin_edges[:-1] + bin_edges[1:]) / 2
    elif bin_centers is None or counts is None:
        raise ValueError("Devi fornire `data`, oppure `bin_centers` e `counts`.")

    sigma_counts = np.sqrt(counts)
    sigma_counts[sigma_counts == 0] = 1.0  # evita divisioni per zero

    # Trova picchi sull'istogramma
    ...
    # Stima iniziale dei parametri per ciascun picco
    initial_params = []
    for peak in peaks:
        amp_guess = counts[peak]
        mu_guess = bin_centers[peak]
        sigma_guess = (bin_centers[1] - bin_centers[0]) * 2
        initial_params += [amp_guess, mu_guess, sigma_guess]

    # Fit globale con somma di gaussiane
    try:
        popt, pcov = curve_fit(sum_of_gaussians, bin_centers, counts, p0=initial_params,
                               sigma=sigma_counts, absolute_sigma=True, maxfev=10000)
    except Exception as e:
        print(f"Errore nel fit globale: {e}")
        return

    perr = np.sqrt(np.diag(pcov))
    n_gauss = len(popt) // 3
    x_fit = np.linspace(bin_centers[0], bin_centers[-1], 2000)
    y_fit_total = sum_of_gaussians(x_fit, *popt)

    # Output dei parametri
    print("Risultati del fit globale:")
    results = []
    for i in range(n_gauss):
        amp, mu, sigma = popt[3*i:3*i+3]
        amp_err, mu_err, sigma_err = perr[3*i:3*i+3]
        results.append((mu, sigma, mu_err, sigma_err))
    results.sort()  # ordina per μ crescente

    for i, (mu, sigma, mu_err, sigma_err) in enumerate(results, 1):
        print(f"{i}. μ = {mu:.3f} ± {mu_err:.3f}, σ = {sigma:.3f} ± {sigma_err:.3f}")

    # Plot finale
    if plot:
        plt.figure(figsize=(10, 6))
        plt.bar(bin_centers, counts, width=(bin_centers[1]-bin_centers[0]), alpha=0.5, label='Dati')

        for i in range(n_gauss):
            amp, mu, sigma = popt[3*i:3*i+3]
            y_gauss = gaussian(x_fit, amp, mu, sigma)
            plt.plot(x_fit, y_gauss, '--', label=f'Gaussiana #{i+1}')

        plt.plot(x_fit, y_fit_total, 'k-', label='Somma totale')
        plt.xlabel("X")
        plt.ylabel("Conteggi")
        plt.title("Fit globale con somma di gaussiane")
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
        plt.show()
